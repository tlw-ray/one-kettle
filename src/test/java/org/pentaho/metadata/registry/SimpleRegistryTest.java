...
public class SimpleRegistryTest {
  ...
  public void testFactory() throws Exception {
    ...
    links =
        metadataRegistry.findObjectLinks( ktr1.getId(), Verb.VERB_POPULATES.getId(), Type.TYPE_PHYSICAL_TABLE.getId() );
    Assert.assertEquals( "id list is wrong size", 1, links.size() );
    Assert.assertEquals( "id is wrong size", table1.getId(), links.get( 0 ).getObjectId() );
    ...
    Assert.assertTrue( ktr1.getId().equals( entities.get( 0 ).getId() )
        || ktr1.getId().equals( entities.get( 1 ).getId() ) || ktr1.getId().equals( entities.get( 2 ).getId() ) );
    Assert.assertTrue( model1.getId().equals( entities.get( 0 ).getId() )
        || model1.getId().equals( entities.get( 1 ).getId() ) || model1.getId().equals( entities.get( 2 ).getId() ) );
    Assert.assertTrue( view1.getId().equals( entities.get( 0 ).getId() )
        || view1.getId().equals( entities.get( 1 ).getId() ) || view1.getId().equals( entities.get( 2 ).getId() ) );
  ...
  public void testDelete() {
    ...
    Link bogusLink =
        new Link( ktr1.getId(), ktr1.getTypeId(), Verb.VERB_POPULATES.getId(), bogus.getId(), bogus.getTypeId() );
