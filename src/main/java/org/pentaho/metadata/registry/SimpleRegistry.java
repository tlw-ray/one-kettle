...
public abstract class SimpleRegistry extends RegistryBase {
   ...
   * @param done
   */
  protected void findAllLinks( Set<String> typeIds, List<Entity> found, Set<String> foundIds, List<Link> targets,
      Set<String> done ) {
    ...
    for ( Link target : targets ) {
      ...
      if ( !foundIds.contains( target.getObjectId() )
          && ( typeIds == null || typeIds.contains( target.getObjectTypeId() ) ) ) {
        found.add( getEntity( target.getObjectId(), target.getObjectTypeId() ) );
        foundIds.add( target.getObjectId() );
      }
      if ( !foundIds.contains( target.getSubjectId() )
          && ( typeIds == null || typeIds.contains( target.getSubjectTypeId() ) ) ) {
